...
\begin{abstract}
Coastal seawall optimization under extreme-value constraints exhibits a special mathematical structure: a \emph{convex chain}---strictly convex objective with tridiagonal Hessian. This structure permits decomposition into pairwise subproblems whose KKT conditions guarantee global optimality, enabling $O(n)$ computational complexity instead of $O(n^3)$ required by standard solvers.

Standard optimization methods fail in the extreme-value regime: global solvers have prohibitive complexity; Monte Carlo gradient descent encounters fundamental sample complexity barriers due to tail rarity. By contrast, the pairwise algorithm achieves global optimality analytically without sampling, providing orders of magnitude computational speedup.

We prove soundness via block coordinate descent theory and validate via numerical experiments on discretized coastlines. The convex chain structure naturally arises from exponential spatial decay of extremal dependence, making this an information-theoretically principled approach to spatial extreme-value optimization.
\end{abstract}
...
\noindent\textbf{Toy Example: Convolution-Based Discretization}

To illustrate the principle, consider a simple continuous optimization problem with spatial coupling:
\[
\min_u \int_0^L \left[ C(u(x)) + (u'(x))^2 \right] dx + \int_0^L \int_0^L K(x, y) u(x) u(y) \, dx \, dy
\]

where $C(u)$ is a local cost, the derivative term penalizes spatial smoothness, and $K(x,y) = e^{-\lambda|x-y|}$ is an exponentially-decaying kernel representing spatial coupling.

Discretize with grid spacing $\Delta x$: $u_i \approx u(x_i)$ at points $x_i = i \Delta x$. The integral $\int_0^L \int_0^L K(x,y) u(x) u(y) dx dy$ becomes
\[
\sum_{i,j} K(x_i, x_j) u_i u_j \, (\Delta x)^2 \approx \sum_i C_i(u_i) + \sum_{i=1}^{n-1} \Phi_{i,i+1}(u_i, u_{i+1})
\]

where $\Phi_{i,i+1}(u_i, u_{i+1})$ captures:
\begin{itemize}
    \item Direct coupling: $K(x_i, x_{i+1}) u_i u_{i+1}$ (large, $O(1)$),
    \item Nearest-neighbor: $K(x_i, x_{i+2}) u_i u_{i+2}$ (decayed, $O(e^{-\lambda \Delta x})$),
    \item Distant coupling: $K(x_i, x_j)$ for $|i-j| \geq 2$ (exponentially small, $O(e^{-2\lambda\Delta x})$).
\end{itemize}

The exponential kernel ensures that non-nearest-neighbor couplings become negligible after discretization, justifying the chain structure. This principle extends to the coastal problem via max-stable processes, where the extremal dependence coefficient plays the role of $K(x,y)$.
...
Under \textbf{Assumption 1}, the pairwise potential $\Phi_{i,i+1}(h_i, h_{i+1})$ is jointly convex. The validity of this assumption in extreme-value settings is well-established (given Assumption 0):
...
Thus Assumption 1 holds in the spatial extreme-value setting, and all subsequent theorems apply.
\end{remark}
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{figure_08_hessian_structure.png}
\caption{\textbf{Figure 8: Hessian Matrix Sparsity Comparison.} Left: The tridiagonal Hessian structure of the pairwise algorithm contains only three non-zero diagonals (main diagonal plus immediate off-diagonals), resulting in $\sim 14.5\%$ sparsity for $n=20$ variables. This structure enables efficient factorization and $O(n)$ computational cost. Right: Standard gradient descent operates on a full dense Hessian with 100\% sparsity, requiring $O(n^3)$ complexity for matrix factorization. The sparsity difference directly translates to computational speedup: for $n=100$, pairwise exploits structure to achieve $562\times$ faster convergence.}
...
\[
\text{Var}\left[\frac{\partial}{\partial h}E[(S-h)_+]\right] \sim \frac{p(1-p)}{N} \approx \frac{p}{N}
\]
...
\subsection{Sample Complexity Barrier: Formal Result}

\begin{theorem}[Sample Complexity for Tail Probability Estimation under Exponential Distribution]
\label{thm:tail_sample_complexity}
Let $S \sim \text{Exp}(\lambda)$ with tail probability $p(h) := P(S > h) = e^{-\lambda h}$. Estimate $p(h)$ using the empirical frequency $\hat{p}(h) = \frac{1}{N}\sum_{k=1}^N \mathbf{1}_{S_k > h}$ from $N$ i.i.d. samples.

To achieve relative error at most $\epsilon$ (i.e., $\mathbb{E}[|\hat{p}(h) - p(h)|/p(h)] \leq \epsilon$), the required sample size is:
\[
N \geq N_{\min}(\epsilon, p) = \frac{C}{p \cdot \epsilon^2}
\]
where $C$ is a universal constant (typically $C \approx 1$ for the $\sqrt{\text{Var}}$ standard, $C \approx 4$ for the $\text{Var}$ standard).

For the extreme-value regime ($p \leq 0.01$) and practical accuracy ($\epsilon = 0.1$):
\[
N_{\min} \geq \frac{1}{0.01 \cdot 0.01} = 10^4 \text{ samples}.
\]

\noindent\textbf{Effective Sample Size:} The key insight is that only approximately $Np$ samples from the dataset actually fall in the tail region $\{S > h\}$. Thus, the effective sample size is $N_{\text{eff}} = Np$. For standard implementations with $N \lesssim 1000$ and $p = 0.01$, we have $N_{\text{eff}} \lesssim 10$, insufficient for reliable tail estimation.
\end{theorem}

\begin{remark}[Extension to Gradient Estimation]
Since the gradient $\nabla_h J = -\frac{\partial}{\partial h} E[(S - h)_+] = -P(S > h) = -p(h)$, gradient estimation inherits the same sample complexity barrier. Any Monte Carlo estimate of $\nabla J$ requires effective tail samples $N_{\text{eff}} = Np$ comparable to the bound in Theorem~\ref{thm:tail_sample_complexity}.

For practical sample budgets ($N \lesssim 1000$) in the extreme-value regime ($p \lesssim 0.01$), gradient estimates are fundamentally unreliable, leading to the high errors (31.6\%--70.7\%) observed in Experiment 7.3.

\noindent\textbf{Scope:} Theorem~\ref{thm:tail_sample_complexity} is stated for the simplified exponential model. More general results under extremal value theory (Fréchet, Weibull, GEV distributions) exist in \cite{DeHaan:ExtremeValue}; all exhibit similar $O(1/(p\epsilon^2))$ scaling due to the rarity of tail events.
\end{remark}
...
\begin{theorem}[Monotone Decrease Property of Pairwise Optimization]
\label{thm:monotone_decrease}
Let $J(h)$ be strictly convex over $\mathbb{R}^n_+$ and suppose we perform a single pairwise minimization step on block $(i, i+1)$:
\[
h^{\text{new}} = \arg\min_{h_i, h_{i+1}} J(h_i, h_{i+1}, h_3^{\text{old}}, \ldots, h_n^{\text{old}})
\]
with all other $h_j$ ($j \notin \{i, i+1\}$) held fixed at their current values. Then:
\[
J(h^{\text{new}}) \leq J(h^{\text{old}})
\]
with equality if and only if $(h_i^{\text{old}}, h_{i+1}^{\text{old}})$ already satisfies the block-wise KKT conditions:
\[
\frac{\partial J}{\partial h_i}\bigg|_{h_i=h_i^{\text{old}}} = 0, \quad \frac{\partial J}{\partial h_{i+1}}\bigg|_{h_{i+1}=h_{i+1}^{\text{old}}} = 0
\]
(with all other variables fixed).
\end{theorem}

\begin{proof}
Since $J$ is strictly convex, the minimization problem over the 2D block $(h_i, h_{i+1})$ is strictly convex. By definition of minimization, $J(h^{\text{new}}) \leq J(h^{\text{old}})$.

Equality holds if and only if $h^{\text{old}}$ is already the minimizer of the 2D subproblem, which occurs if and only if the gradient of $J$ with respect to $(h_i, h_{i+1})$ vanishes at $h^{\text{old}}$, i.e., the block-wise KKT conditions are satisfied.

Therefore, unless the block is already optimized (KKT-satisfied), the pairwise update strictly decreases $J$.
\end{proof}

\noindent\textbf{Practical Implication:} This theorem guarantees that each pairwise sweep makes progress toward the global optimum. Combined with strict convexity and level-boundedness (Assumption 4), the sequence of pairwise sweeps converges monotonically to the unique global optimum, as established by Tseng's BCD convergence theory.
...
\begin{enumerate}
    \item Strict convexity of $J(h) = \sum_i C_i(h_i) + \sum_i \Phi_{i,i+1}(h_i, h_{i+1})$ is inherited from strict convexity of $C_i$ and convexity of $\Phi_{i,i+1}$ (Assumption 1).
...
All eight validation figures are generated by the script \texttt{generate\_revision\_figures\_experiments.py} and included in the directory \texttt{results/}:
...
\begin{enumerate}
    ...
    \item \textbf{Figure 8}: Hessian Matrix Sparsity Comparison (tridiagonal vs. dense structure).
\end{enumerate}
