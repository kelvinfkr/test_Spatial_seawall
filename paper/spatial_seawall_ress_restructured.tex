...
We demonstrate that standard gradient descent methods face fundamental limitations in the extreme-value regime: when tail probability $p \ll 1$, the effective sample size scales as $N_{\text{eff}} = N \cdot p$, causing gradient variance to scale as $\Var[\nabla J] \sim O(1/(Np))$. Monte Carlo gradient descent with $N=200$ produces gradient errors exceeding 70\%, preventing convergence. Analytical pairwise decomposition, by contrast, avoids sampling entirely, achieving 912× speedup (0.08~s vs 73~s for CVX) while maintaining provable global optimality.

We validate the approach through four experiments: (1) pairwise algorithm matches CVX global solver to 0.1~ft precision; (2) discretization converges with grid refinement (error $<0.2\%$ for $n>50$); (3) Monte Carlo gradient descent baseline confirms theoretical limitations; (4) correlation sensitivity analysis quantifies spatial effects (height increase up to 0.17~ft, cost increase up to 14\% under $\rho=0.85$ correlation). Seven figures illustrate extremal dependence decay, system reliability, gradient variance bounds, algorithm convergence, and multi-scale validation. This work demonstrates that pairwise analytical decomposition substantially outperforms sampling-based methods in the extreme-value regime, with performance validated across multiple scales and scenarios.
\end{abstract}
...
\begin{proposition}[Joint Convexity of Pairwise Potential]
The pairwise potential $\Phi_{i,i+1}(h_i, h_{i+1}) = D_i(h_i) + D_{i+1}(h_{i+1}) + \Lambda_{i,i+1}(h_i, h_{i+1})$ is jointly convex in $(h_i, h_{i+1})$, and the Hessian $\nabla_{(h_i, h_{i+1})}^2 \Phi_{i,i+1}$ is positive semi-definite.
\end{proposition}

\begin{proof}
We verify convexity of each component:

\noindent\textbf{Marginal damage terms:} For any location $i$, the damage function is:
\[
...
Taking derivatives:
\[
\frac{\partial D_i}{\partial h_i} = -P(S_i > h_i), \quad \frac{\partial^2 D_i}{\partial h_i^2} = f_{S_i}(h_i) > 0
\]

Thus each $D_i(h_i)$ is strictly convex in $h_i$ alone.

\noindent\textbf{Interaction penalty $\Lambda_{i,i+1}$:} Let $p_i := P(S_i > h_i)$ and $p_{i+1} := P(S_{i+1} > h_{i+1})$. Then:
\[
\Lambda_{i,i+1} = \theta \left[ P(S_i > h_i, S_{i+1} > h_{i+1}) - p_i p_{i+1} \right]
\]

Since the joint tail probability $P(S_i > h_i, S_{i+1} > h_{i+1})$ depends on extremal dependence and is a convex decreasing function of both $h_i$ and $h_{i+1}$ (as larger heights reduce the joint tail event), and since $p_i p_{i+1}$ is a separable product of convex terms, the difference is convex in the product space \cite{Boyd:Convex}. Thus $\Lambda_{i,i+1}$ is convex.

\noindent\textbf{Sum:} The sum of convex functions is convex, so:
\[
\nabla_{(h_i, h_{i+1})}^2 \Phi_{i,i+1} = \nabla^2 D_i + \nabla^2 D_{i+1} + \nabla^2 \Lambda_{i,i+1} \succeq 0
\]
\end{proof}
...
\subsection{Heuristic Argument: Sample Complexity in Extreme-Value Regime}

\begin{remark}[Sample Complexity Scaling in Tail Estimation]
In the extreme-value regime ($p \ll 1$), estimating tail-dependent quantities (such as gradients of tail integrals) requires sufficient samples in the tail region itself. A heuristic argument follows: only approximately $Np$ out of $N$ samples exceed the threshold $h$, giving effective sample size $N_{\text{eff}} = Np$. To estimate tail quantities to accuracy $\epsilon$, standard estimation theory suggests requiring $N_{\text{eff}} \geq O(1/\epsilon^2)$, which implies:
\[
N \geq O\left(\frac{1}{p \cdot \epsilon^2}\right)
\]

For $p=0.01$ and target accuracy $\epsilon = 0.1$ (i.e., 10\% relative error in gradient), this estimates $N \gtrsim O(10^4)$ samples. While this is a heuristic argument rather than a complete theorem (see \cite{DeHaan:ExtremeValue} for rigorous asymptotic results), it provides intuition for why Monte Carlo methods become impractical in the extreme-value regime.
\end{remark}
